...
const css_variable_manager_1 = require("../css-variable-manager");
...
class ColorManager {
    static colorToName(color) {
        ...
        return (0, string_1.toDashCase)(nearest(hex).name);
    }
    static detectColorType(color) {
        if (color.startsWith("#"))
            return "Hex";
    ...
    static colorToHex(color) {
        ...
        switch (type) {
            ...
            case "HSL":
            case "HSLA":
                const baseHex = new colorjs_io_1.default(color.replace(/_/gi, " ")).to("srgb");
    ...
    static extract(folderPath) {
        ...
        const regex = /\b(?:bg|text|border|ring|shadow|outline)-\[(#(?:[0-9a-fA-F]{3}|[0-9a-fA-F]{6})|rgb\(\d{1,3},\d{1,3},\d{1,3}\)|rgba\(\d{1,3},\d{1,3},\d{1,3},(?:0|1|0?\.\d+)\)|hsl\(\d{1,3},\d{1,3}%,\d{1,3}%\)|hsla\(\d{1,3},\d{1,3}%,\d{1,3}%,(?:0|1|0?\.\d+)\)|oklch\(\d{1,3}(?:\.\d+)?%?_+\d{1,3}(?:\.\d+)?_+\d{1,3}(?:\.\d+)?(?:_\/?_?(?:0|1|0?\.\d+))?\))\]/gi;
